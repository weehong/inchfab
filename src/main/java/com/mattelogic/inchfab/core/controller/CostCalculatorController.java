...
public class CostCalculatorController {
  ...
  public ResponseEntity<ApiResponseDto<ResultResponseDto>> calculate(
      @Valid @RequestBody CostRequestDto costRequestDto) throws Throwable {
    log.debug("REST request to calculate cost: {}", costRequestDto);
    return ResponseEntity.ok(service.calculate(costRequestDto));
  }
}
